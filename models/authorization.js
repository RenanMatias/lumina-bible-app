...
function validateUser(user) {
  if (!user || !user.features) {
    throw new InternalServerError({
      cause: "You must provide `user` to the `authorization` model.",
    });
  }
}
...
function validateFeature(feature) {
  if (!feature || !availableFeatures.includes(feature)) {
    throw new InternalServerError({
      cause: "You must provide a known `feature` to the authorization model.",
    });
  }
}
...
function validateResource(resource) {
  if (!resource) {
    throw new InternalServerError({
      cause: "You must provide a `resource` to `authorization.filterOutput()`.",
    });
  }
}
